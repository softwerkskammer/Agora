...
activitystore.allActivities((err, activities) => {
  ...
  more.forEach((activity) =>
    console.log(activity.title() + " (url: " + activity.url() + "-" + activity.resourceNames() + ")")
  );
  ...
  more.forEach((activity) => {
    ...
    const newRegistrations = R.uniqBy(
      (registration) => registration.memberId,
      R.flatten(R.keys(resourcesPlain).map((name) => resourcesPlain[name]._registeredMembers))
    );
