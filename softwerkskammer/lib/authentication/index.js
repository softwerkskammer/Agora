...
function setReturnOnSuccess(req, res, next) {
  ...
  res.cookie("loginChoice", loginChoiceCookieFor(decodeURIComponent(req.url)), {
...
function localStrategyCallback(req, res, next) {
  passport.authenticate(localStrategy.name, (err, user, problemMessage) => {
    ...
    if (problemMessage) {
      statusmessage.errorMessage("authentication.error", problemMessage).putIntoSession(req);
    }
    if (!user) {
