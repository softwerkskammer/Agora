...
function updateDatesInActivity(activity) {
  const start =
    activity.state.startUnix && !isNaN(activity.state.startUnix)
      ? DateTime.fromMillis(activity.state.startUnix * 1000)
      : DateTime.local();
  activity.state.startDate = start.set({ millisecond: 0, second: 0 }).setZone("Europe/Berlin").toJSDate();

  const end =
    activity.state.endUnix && !isNaN(activity.state.endUnix)
      ? DateTime.fromMillis(activity.state.endUnix * 1000)
      : DateTime.local();
