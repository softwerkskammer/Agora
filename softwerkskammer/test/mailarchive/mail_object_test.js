...
describe('Mail', () => {
  ...
  const mailWithFrom = new Mail({
    id: 'Mail 1',
    from: {name: 'name', address: 'local@domain'}
  });

  it('restores time from unix time', () => {
    const expectedTime = moment('01 Jan 2010 21:14:14 +0100', 'DD MMM YYYY HH:mm:ss Z');
    const mail = new Mail({
      id: 'Mail 1',
  ...
  it('contains member ID if it is available', () => {
    const mail = new Mail({
      id: 'Mail 1',
      from: {id: 'id'}
    });
  ...
  it('sets member ID to null if it is not available', () => {
    const mail = new Mail({
      id: 'Mail 1'
    });
