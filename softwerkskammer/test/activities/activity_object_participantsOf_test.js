...
describe('Activity\'s participantsOf function', function () {
  it('returns an empty list if no participants are present', function () {
    var activity = new Activity({resources: {Veranstaltung: { _registeredMembers: [ {memberId: 'memberId1'}]}}});
  ...
  it('returns no participant if he is not registered for the resource in question', function () {
    var activity = new Activity({resources: {Veranstaltung: { _registeredMembers: [ {memberId: 'memberId2'}]}}});
    activity.participants = [member1];
    expect(activity.participantsOf('Veranstaltung')).to.have.length(0);
  });
  ...
  it('returns one participant if only one is participant of the activity', function () {
    var activity = new Activity({resources: {Veranstaltung: { _registeredMembers: [ {memberId: 'memberId2'}, {memberId: 'memberId1'}]}}});
