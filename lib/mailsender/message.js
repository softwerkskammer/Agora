...
Message.prototype.setBccToGroupMemberAddresses = function (groups) {
  this.setBccToMemberAddresses(_.chain(groups).compact().pluck('members').flatten().value());
};

Message.prototype.setBccToMemberAddresses = function (members) {
  this.bcc = _.chain(members).compact().map(function (member) { return member.email(); }).uniq().value();
};
