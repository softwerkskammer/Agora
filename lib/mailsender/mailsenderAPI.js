...
module.exports = {
  ...
  dataForShowingMessageForActivity: function (activityURL, globalCallback) {
    async.parallel({
        activity: function (callback) { activitystore.getActivity(activityURL, callback); },
  ...
  sendMailToParticipantsOf: function (activityURL, message, callback) {
    var type = 'Erinnerung';
    return activitystore.getActivity(activityURL, function (err, activity) {
  ...
  sendRegistrationAllowed: function (member, activity, waitinglistEntry, callback) {
    ...
    var markdown = 'Für die Veranstaltung ["' + activity.title() + '"](' + activityFullUrl + ') sind wieder Plätze frei.\n\nDu kannst Dich bis ' +
      waitinglistEntry.registrationValidUntil() + ' Uhr erneut registrieren.';
    var message = new Message();
    message.setReceiver(member);
