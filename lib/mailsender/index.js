          ...
          mailsenderAPI.sendMail(message, function (err) {
            if (err) {
              statusmessage.errorMessage('Email nicht gesendet',
                'Deine Nachricht wurde nicht gesendet. Grund: ' + err).putInSession(req);
            } else {
              statusmessage.successMessage('Email gesendet', 'Deine Einladung ist unterwegs').putInSession(req);
            }
      ...
      return membersAPI.getMember(req.body.nickname, function (err, member) {
        ...
        mailsenderAPI.sendMail(message, function (err) {
          if (err) {
            ...
            statusmessage.successMessage('Email gesendet', 'Deine Nachricht ist unterwegs').putInSession(req);
          }
          res.redirect(req.body.successURL);
        });
      });
