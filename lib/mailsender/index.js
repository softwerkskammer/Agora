...
module.exports = function (app) {
  ...
  function messageSubmitted(req, res) {
    ...
    function processResult(err, type) {
      err ?
        statusmessage.errorMessage('message.title.email_problem', 'message.content.mailsender.error_reason', {type: type, err: err.toString()}).putIntoSession(req) :
        statusmessage.successMessage('message.title.email_successful', 'message.content.mailsender.success', {type: type}).putIntoSession(req);
  ...
  function resignmentSubmitted(req, res) {
    ...
    return mailsenderAPI.sendResignment(markdown, req.user.member, function (err, type) {
      err ?
        statusmessage.errorMessage('message.title.email_problem', 'message.content.mailsender.error_reason', {type: type, err: err.toString()}).putIntoSession(req) :
        statusmessage.successMessage('message.title.email_successful', 'message.content.mailsender.success', {type: type}).putIntoSession(req);
  ...
  app.get('/resign/:nickname', function (req, res) {
    var questions = '**Warum willst du austreten?**\n<antwort hier>\n\n**Was möchtest Du uns mitteilen (Anregungen, Kritik, Lob)?**\n<antwort hier>';
    res.render('compose-resign', {nickname: req.params.nickname, markdown: questions});
  });
