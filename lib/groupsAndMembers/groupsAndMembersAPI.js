...
module.exports = {
  ...
  updateAdminlistSubscriptions: function (member, callback) {
    if (!member.subscribedGroups) {
      return callback();
    }
  ...
  saveGroup: function (group, callback) {
    ...
    groupsAPI.createOrSaveGroup(group, function (err, newGroup, existingGroup) {
      ...
      if (existingGroup) {
        ...
        membersToCheck = _.union(addedOrganizers, removededOrganizers);
      } else {
        membersToCheck = newGroup.organizers;
      }
      async.each(membersToCheck, function (memberID, callback) {
        self.getMemberWithHisGroupsByMemberId(memberID, function (err, member) {
          self.updateAdminlistSubscriptions(member, callback);
        });
      });
      callback(null);
    });
