...
module.exports = {
  mailHeaders: function (group, callback) {
    persistence.listByFieldWithOptions(
      {group: group}, {text: 0, html: 0}, {timeUnix: -1},
      function (err, mails) {
  ...
  mailForId: function (id, callback) {
    persistence.getById(id, function (err, mail) {
      if (err) { return callback(err); }
      return callback(null, new Mail(mail));
    });
  },
  ...
  mailHeadersWithProfileData: function (group, callback) {
    var module = this;
    module.mailHeaders(group, function (err, theHeaders) {
      if (err != null) {return callback(err, theHeaders); }
      module.addProfileDataForMembers(theHeaders, function (err) {
        callback(err, theHeaders);
      });
    });
  },
  ...
  sortOnLastResponseTimeDescending: function (mail1, mail2) {
    return mail2.lastResponse.timeUnix - mail1.lastResponse.timeUnix;
  },

  threadedMails: function (group, sort, callback) {
    this.mailHeadersWithProfileData(group, function (err, mailHeaders) {
      if (err) {
        callback(err);
        return;
      }
