  ...
  persistence = {
    ...
    remove: function (object, callback) {
      ...
      performInDB(function (err, db) {
        if (err) { return callback(err); }
        var collection = db.collection(collectionName);
        collection.remove({id: object.id}, {w: 1}, function (err) {
          if (err) { return callback(err); }
          logger.info(object.constructor.name + ' removed: ' + JSON.stringify(object));
          callback(null);
        });
      });
    },
    ...
    openDB: function () {
      ...
      var dbName = conf.get('mongoDB');

      var theDB = new Db(dbName, new Server(host, port), {w: 1, safe: false});
