  ...
  var persistence = {
    ...
    save: function (object, callback) {
      ...
      performInDB(function (err, db) {
        ...
        collection.update({id: object.id}, object, {upsert: true}, function (err) {
          if (err) { return callback(err); }
    ...
    saveWithVersion: function (object, callback) {
      ...
      performInDB(function (err, db) {
        ...
        object.version = currentVersion ? currentVersion + 1 : 1;
        self.getById(object.id, function (err, result) {
          if (err) { return callback(err); }
          if (result) { // object exists
            collection.findAndModify({id: object.id, version: currentVersion}, [], object, {new: true, upsert: false}, function (err, newObject) {
              if (err) { return callback(err); }
              if (!newObject) { return callback(new Error("Object not found.")); }
              logger.info(object.constructor.name + " saved: " + JSON.stringify(object));
