...
module.exports = {
  ...
  BLOG_ENTRY_REGEX: /blog_(\d{4}-\d{1,2}-\d{1,2})/,
  ...
  getBlogpostsForGroup: function (groupname, callback) {
    ...
    Git.lsblogposts(groupname, this.BLOG_ENTRY_FILE_PATTERN, function (err, result) {
      ...
      _.filter(result, function (item) { return item.match(self.BLOG_ENTRY_REGEX) !== null; });

      async.map(result,
        function (path, mapCallback) {
          Git.readFile(path, 'HEAD', function (err, post) {
            if (err) { return mapCallback(err); }
            mapCallback(null, misc.parseBlogPost(path, post, self.BLOG_ENTRY_REGEX));
          });
        },
  ...
  listChangedFilesinDirectory: function (directory, callback) {
    ...
    Git.log(directory, 'HEAD', 30, function (err, metadata) {
      var datas = _(metadata).uniq('name').reject(function (item) { return item.name.match(self.BLOG_ENTRY_REGEX); }).value();
      callback(null, datas);
    });
  }
