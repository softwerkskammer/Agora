...
module.exports = {
  ...
  parseBlogPost: function (path, post, regex) {
...
//needed?      _.filter(result, function (item) { return item.match(self.BLOG_ENTRY_REGEX) !== null; });

      async.map(result,
        function (path, mapCallback) {
          Git.readFile(path, 'HEAD', function (err, post) {
            if (err) { return mapCallback(err); }
            mapCallback(null, self.parseBlogPost(path, post, wikiObjects.BLOG_ENTRY_REGEX));
          });
        },
