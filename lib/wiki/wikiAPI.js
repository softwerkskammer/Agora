...
var FileWithChangelist = beans.get('wikiObjects').FileWithChangelist;
var DirectoryWithChangedFiles = beans.get('wikiObjects').DirectoryWithChangedFiles;
            ...
            Git.latestChanges(item.fullname + '.md', moment, function (err, metadata) {
              ...
              if (metadata.length > 0) {
                Git.diff(item.fullname + '.md', 'HEAD@{' + moment.toISOString() + '}..HEAD', function (err, diff) {
                  if (err) { return itemsCallback(err); }
                  resultLine.addFile(new FileWithChangelist({dir: directory, file: item.name, changelist: metadata, diff: new Diff(diff)}));
                  itemsCallback();
                });
              } else { itemsCallback(); }
            });
