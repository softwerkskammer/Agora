...
module.exports = function (app) {
  ...
  app.post('/:subdir/:page', function (req, res, next) {
    ...
    api.pageSave(subdir, pageName, req.body, req.user.member, function (err, conflict) {
      if (err) {
        statusmessage.errorMessage(req, 'message.title.problem', 'message.content.save_error');
        return next(err);
      }
      conflict ?
        statusmessage.errorMessage(req, 'message.title.conflict', 'message.content.wiki.conflict') :
        statusmessage.successMessage(req, 'message.title.save_successful', 'message.content.wiki.saved');
  ...
  app.post('/search', function (req, res, next) {
    ...
    if (searchtext.length < 2) {
      statusmessage.errorMessage(req, 'message.title.query', 'message.content.query');
