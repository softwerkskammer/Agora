...
function showPage(subdir, pageName, pageVersion, req, res, next) {
  ...
  api.pageShow(completePageName, pageVersion, function (err, content) {
    if (err || !content) {
...
module.exports = function (app) {
  ...
  app.post('/:subdir/:page', function (req, res, next) {
    var pageName = req.params.page;
    var subdir = req.params.subdir;
    api.pageSave(subdir, pageName, req.body, req.user, function (err, conflict) {
      if (err) {
        statusmessage.errorMessage('Problem', 'Das Speichern war nicht erfolgreich').putInSession(req);
        return next(err);
      }
      var message =
        conflict ?
          statusmessage.errorMessage('Speicherkonflikt', 'Deine Änderungen wurden gespeichert. ' +
            'Du hast allerdings Änderungen eines anderen Benutzers überschrieben. Bitte überprüfe nochmals die Wikiseite.') :
          statusmessage.successMessage('Speichern erfolgreich', 'Deine Änderungen wurden gespeichert.');
      if (message) { message.putInSession(req); }
