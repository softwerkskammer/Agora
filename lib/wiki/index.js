...
module.exports = function (app) {
  ...
  app.post('/:subdir/:page', function (req, res, next) {
    ...
    api.pageSave(subdir, pageName, req.body, req.user.member, function (err, conflict) {
      if (err) {
        statusmessage.errorMessage('message.title.problem', 'message.content.save_error').putInSession(req, res);
        return next(err);
      }
      var message =
        conflict ?
          statusmessage.errorMessage('message.title.conflict', 'message.content.wiki.conflict') :
          statusmessage.successMessage('message.title.save_successful', 'message.content.wiki.saved');
  ...
  app.post('/search', function (req, res, next) {
    ...
    if (searchtext.length < 2) {
      statusmessage.errorMessage('message.title.query', 'message.content.query').putInSession(req, res);
