...
module.exports = {
  ...
  log: function (path, version, howMany, callback) {
    gitExec(['log', '-' + howMany, '--reverse', '--no-notes', '--pretty=format:%h%n%H%n%an%n%ai%n%s', version, '--', path], function (err, data) {
      var logdata = data ? data.toString().split('\n') : [];
      var group;
      var metadata = [];
      for (var i = Math.floor(logdata.length / 5); i-- > 0;) {
  ...
  lsblogposts: function (groupname, callback) {
    gitExec([ 'ls-files', groupname + '/blog_*' ], function (err, data) {
      callback(null, data.toString().split('\n').filter(function (v) {
        return v !== '';
      }));
    });
