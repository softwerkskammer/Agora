...
module.exports = function () {
  ...
  return {
    ...
    lsblogposts: function (groupname, callback) {
      if (!workTree) {
      return callback(null, []);
      }
      gitExec([ 'ls-files', groupname + "/" + conf.stores.wiki.blogPattern ], function (err, data) {
        var blogPosts = data.toString().split('\n').filter(function (v) {
          return v !== '';
        }).map(function (filename) {
            var post =  gitExec(['show', "HEAD" + ':' + filename], function (err, data) {
            if (err)
              callback(err);
            else {
            ...
            console.log("exec:" + JSON.stringify(post));
            return post;
        });
        console.log("git:" + JSON.stringify(blogPosts));
