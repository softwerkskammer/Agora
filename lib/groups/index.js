...
module.exports = function (app, conf) {
    ...
    groupsAndMembers.getGroupAndUsersOfList(req.params.groupname, function (err, group, users) {
      if (err) {
        ...
        res.render('get', {group: group, users: users,
          userIsRegistered: false && userIsRegistered(req),
  ...
  app.get('/checkgroupname', function (req, res) {
    var groupname = req.query.longName;
    groups.isGroupNamePresent(groupname, function (err, result) {
      if (err) {
        //TODO hgp Passt das wirklich?
        return 'true';
      }
