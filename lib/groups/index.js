...
module.exports = function (app) {
  ...
  function groupSubmitted(req, res, next) {
    ...
    groupsAPI.createOrSaveGroup(group, function (err, newGroup, existingGroup) {
      if (err) { return next(err); }

      var addedOrganizers = _.difference(newGroup.organizers, existingGroup.organizers);
      var removededOrganizers = _.difference(existingGroup.organizers, newGroup.organizers);
      var membersToCheck = _.union(addedOrganizers, removededOrganizers);
      async.each(membersToCheck, function (memberID, callback) {
        groupsAndMembers.getMemberWithHisGroupsByMemberId(memberID, function (err, member) {
          groupsAndMembers.updateAdminlistSubscriptions(member, callback);
        });
      });

      statusmessage.successMessage('message.title.save_successful', 'message.content.groups.saved').putInSession(req);
