...
module.exports = function (app) {
  ...
  app.get('/', function (req, res, next) {
    groupsAPI.getAllAvailableGroups(function (err, groups) {
      if (err) { return next(err); }
      async.map(groups, function (group, callback) { groupsAndMembers.addMembersToGroup(group, callback); },
        function (err, groupsWithMembers) {
          if (err) { return next(err); }
          var regionalgroups = groupsWithMembers.filter(function (group) { return group.type === group.allTypes()[1]; });
          var themegroups = groupsWithMembers.filter(function (group) { return group.type === group.allTypes()[0]; });
