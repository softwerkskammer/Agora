...
module.exports = function (app) {
  ...
  app.get('/', function (req, res, next) {
    groupsAPI.getAllAvailableGroups(function (err, groups) {
      if (err) { return next(err); }
      res.render('index', {regionalgroups: Group.regionalsFrom(groups), themegroups: Group.thematicsFrom(groups)});
    });
  });

  app.get('/new', function (req, res) {
    res.render('edit', { group: new Group(), allTypes: Group.allTypes() });
  });
  ...
  app.get('/edit/:groupname', function (req, res, next) {
    groupsAPI.getGroup(req.params.groupname, function (err, group) {
      if (err) { return next(err); }
      res.render('edit', { group: group ? group : new Group(), allTypes: Group.allTypes()});
    });
  });
