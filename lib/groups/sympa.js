...
var createClient = function (clientConfig) {
  ...
  var getAssignedLists = function (callback) {
    var parameters;

    var dataAdapter = function (err, result) {
      var items = result.return;
      callback(err, items);
    }

    // the user should be handed over by the clientConfig
    initializeRequest('complexWhich', remoteAppUser, parameters, dataAdapter);
  };

  client.getAssignedLists = getAssignedLists;
