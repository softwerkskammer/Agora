...
module.exports = {
  ...
  isGroupValid: function (group) {
    ...
    this.getGroup(group.id, function (err, existingGroup) {
      if (!existingGroup) {
        this.isGroupNameAvailable(group.id, function (err, result) {
          if (err) {
        ...
        this.isEmailPrefixAvailable(group.emailPrefix, function (err, result) {
          if (err) {
