    ...
    Git.lsblogposts(groupname, function (err, result) {
      ...
      result.map(function (path) {
        Git.readFile(path, 'HEAD', function (err, rawPost) {
          ...
          unsortedPosts.push(parsedPost);
          if (unsortedPosts.length === result.length) {
            var postsSortedByDate = _.sortBy(unsortedPosts, function (post) {
              // _.sortBy is always in ascending order and we want the latest post first, so use negation
              return -post.date.getTime();
            });
            callback(err, postsSortedByDate);
          }
        });
      });
    });
