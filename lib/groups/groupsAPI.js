...
module.exports = function (conf) {
  ...
  return {
    ...
    isGroupNamePresent: function (groupname, callback) {
      var trimmedGroupName = groupname.trim();
      this.getAllAvailableGroups(function (err, groups) {
        //Falls ein Fehler vorliegt, können wir keine Aussage treffen, wir gehen davon aus dass die Gruppe schon da ist.
        if (err) {
          return callback(err);
        }
        else {
          groups.forEach(function (group) {
            if (group.longName === trimmedGroupName) {
