...
module.exports = {
  ...
  updateAdminListSubscription: function (userMail, isAdmin, callback) {
    ...
    var self = this;
    sympaCache.getSubscribedListsForUser(userMail, function (err, subscribedLists) {
  ...
  getBlogposts : function (groupname) {
    return Git.lsblogposts(groupname, function(err, result) {
      if (err) {
        return err;
      }
      console.log(JSON.stringify(result));
      return result.map(function (rawPost) {
        return convertGitBlogPost(rawPost);
      });
    });
  },

  convertGitBlogPost: function (blogpostsFromGit, callback) {
    var splitted = blogpostsFromGit.split('\n');
    console.log(splitted);
    var title = splitted[0].split(',')[0];
    var date = new Date(splitted[0].split(',')[1]);
    var teaser = splitted[2];
    var text = splitted.slice(2).join("");
