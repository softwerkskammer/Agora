...
app.post('/:activityResultName/upload', function (req, res) {
  new Form().parse(req, function (err, fields, files) {
    ...
    galleryService.storeImage(image.path, function (err, imageUri) {
      if (err) {
        throw err;
      }
      galleryService.getMetadataForImage(imageUri, function (err, metadata) {
        var date;
...
app.post('/:activityResultName/photo/:photoId/edit', function (req, res) {
  var photoData = {
    ...
    tags: _.isArray(req.body.tag) ? req.body.tag : [req.body.tag]
  };
...
app.get('/:activityResultName', function (req, res) {
  ...
  activityresultsService.getActivityResultByName(activityResultName, function (err, activityResult) {
    ...
    var groupedByDay = _.groupBy(_.sortBy(activityResult.photos, sortbyTimestamp), groupByDay);
