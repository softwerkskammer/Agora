...
app.get('/socrates', function (req, res) {
  res.render('socrates', {
    activityResultName: 'SoCraTes 2014',
    columns: {
      0: {
        columnTitle: '#Hopper',
        images: [
...
app.post('/:activityResultName/photo/:photoId/edit', function (req, res) {
  var photoData = {
    ...
    timestamp: new Date(req.body.date + ' ' + req.body.time),
    tags: req.body.tag
  };
...
app.get('/:activityResultName', function (req, res) {
  activityresultsService.getActivityResultByName(req.params.activityResultName, function (err, activityResult) {
    ...
    if (err || !activityResult || !activityResult.id) {
      res.status(NOT_FOUND);
	console.log("Foooooo");
      return res.render('notFound', {
        createUri: app.path(),
