...
module.exports = function (app) {
  ...
  app.post('/memberChanged', function (req, res) {
    ...
    var id = req.body.pk;
    membersAPI.updateMembersFieldWith(id, field, req.body.value, function (successful, errors) {
      if (!successful) { return res.send(500, errors.join(', ')); }
      if (field === 'isAdmin') {
        var isAdmin = !!parseInt(req.body.value, 10);
        membersAPI.getMemberForId(id, function (err, member) {
          if (err) {return res.send(500, err); }
          groupsAPI.updateAdminListSubscription(member.email, isAdmin, function () {
            res.send(200, "OK");
          });
        });
      }
  ...
  app.get('/announcementTable', function (req, res, next) {
    announcementAPI.allAnnouncements(function (err, announcements) {
      ...
      res.render('announcementTable', { announcementsExpired: Announcement.AnnouncementsExpired(announcements), announcementsUntilToday: Announcement.AnnouncementsUntilToday(announcements) });
    });
  });
