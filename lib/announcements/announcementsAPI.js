...
module.exports = {
  ...
  convertDatesBeforeSaving: function (announcement, callback) {
    // Convert German dates to unix timestamp before saving
    if (announcement.fromDate) {
      // Check if we have a German formatted date (administration) and convert it. Otherwise it is already unix (new announcememt)
      if (moment(announcement.fromDate, 'DD.MM.YYYY').isValid()) {
        announcement.fromDate = moment(announcement.fromDate, 'DD.MM.YYYY').unix();
      }
    }
  ...
  saveAnnouncement: function (announcement, callback) {
    ...
    this.convertDatesBeforeSaving(announcement);
