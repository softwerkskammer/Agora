...
module.exports = {
  waitinglistFor: function (activityUrl, callback) {
    activitystore.getActivity(activityUrl, function (err, activity) {
      waitinglistEnrichedWithNicknameAndActivityUrl(callback)(err, activity.allMembersOnWaitinglist());
  ...
  saveWaitinglistEntry: function (args, callback) {
    async.parallel({
      ...
      function (err, results) {
        ...
        results.activity.resources().named(args.resourcename).addMemberIdToWaitinglist(results.member.id, moment().toDate());
