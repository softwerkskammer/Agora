...
module.exports = function (app) {
  ...
  app.post('/add', function (req, res, next) {
    ...
    accessAllowedTo(activityUrl, res, function (err, activity) {
      ...
      if (!resourcename) { resourcename = activity.resourceNames()[0]; }
      var args = {nickname: req.body.nickname, activityUrl: activityUrl, resourcename: resourcename, hoursstring: req.body.registrationValidForHours};
  ...
  app.post('/allowRegistration', function (req, res, next) {
    ...
    accessAllowedTo(activityUrl, res, function (err, activity) {
      ...
      async.eachSeries(rows, waitinglistAPI.allowRegistrationWaitinglistEntry, function (err) {
        if (err) { return next(err); }
        res.redirect('/waitinglist/' + encodeURIComponent(activityUrl));
      });
    });
