...
module.exports = {
  ...
  payWithCreditCard: function (activityUrl, amount, memberId, stripeId, callback) {
    var savePayment = _.partial(saveCreditCardPayment, activityUrl, memberId);
    activitystore.getActivity(activityUrl, function (err, activity) {
      if (err || !activity) { return callback(err); }
      paymentService.payWithCreditCard(savePayment, amount, activity.title(), memberId, stripeId, callback);
    });
  },
