...
module.exports = {
  ...
  addonLinesOfUnsubscribedMembers: function (activityWithParticipants, callback) {
    var subscribedMemberIds = _.map(activityWithParticipants.participants, function (participant) {
    ...
    async.map(unsubscribedMemberIds,
      function (unsubscribedMemberId, cb) {
        memberstore.getMemberForId(unsubscribedMemberId, function (err, unsubscribedMember) {
          if (err) { return cb(err); }
          cb(null, {member: unsubscribedMember, addon: activityWithParticipants.addonForMember(unsubscribedMemberId)});
        });
      }, callback);
  },
