...
module.exports = {
  ...
  upcomingActivitiesForGroupIdsAndMemberId: function (groupIds, memberId, callback) {
    ...
    var map = function () {
      var containsMemberId = function (obj) {
        return true;
        /*
        var resource;
        for (resource in obj.resources) {
          return obj.resources.hasOwnProperty(resource) && resource._registeredMembers.indexOf(memberId) > -1;
        }
        return false;
        */
      };
      emit(containsMemberId(this), this);
    };
    ...
    persistence.mapReduce(map, reduce, {out: {inline: 1}, jsMode: true}, function (err, collection) {
      if (err || !collection || collection.length === 0) { return callback(err); }
      return toActivityList(callback, null, collection[0].value);
    });

      //_.partial(toActivityList, callback));
