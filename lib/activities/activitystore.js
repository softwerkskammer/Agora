  ...
  if (!activitystore) {
    ...
    activitystore = {
      allActivities    : function (callback) {
        persistence.list({activityDate: 1}, async.apply(toActivityList, callback));
      },
      allActivitiesByField : function (callback) {
        persistence.listByField({activityDate : { $gt: "1212/12/12", $lt: "2999/09/09" }}, {activityDate: 1}, async.apply(toActivityList, callback));
      },
