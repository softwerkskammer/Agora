...
module.exports = {
  ...
  addVisitorTo: function (memberId, activityUrl, resourceName, moment, callback) {
    ...
    store.getActivity(activityUrl, function (err, activity) {
      ...
      var waitinglistEntry = activity.resourceNamed(resourceName).waitinglistEntryFor(memberId);
      var canSubscribe = waitinglistEntry ? waitinglistEntry.canSubscribe() : false;
      if (activity.resourceNamed(resourceName).isRegistrationOpen() || canSubscribe) {
        activity.addMemberId(memberId, resourceName, moment);
        return self.saveActivity(activity, callback);
      }
      return callback(null, null, 'Die Anmeldung ist momentan nicht möglich.', 'Die Anmeldung ist noch nicht freigegeben, oder alle Plätze sind belegt.');
    });
  },
