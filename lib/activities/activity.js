...
function Activity(object) {
  if (object) {
    ...
    this.resources = [new Resource(misc.toArray(
        object.resources && object.resources[0] ? object.resources[0]._registeredMembers : object.registeredMembers))];
  } else {
...
Activity.prototype.registeredMembers = function () {
  return this.resources[0]._registeredMembers;
};

Activity.prototype.addMemberId = function (memberId) {
  if (!this.registeredMembers()) {
    this.resources[0]._registeredMembers = [];
  }
  if (this.registeredMembers().indexOf(memberId) === -1) {
    this.resources[0]._registeredMembers.push(memberId);
  }
};
...
Activity.prototype.resetForClone = function () {
  ...
  result.resources[0]._registeredMembers = [];
