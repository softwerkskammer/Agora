...
var AddOnInfoOfMember = beans.get('addOnInfoOfMember');
...
function Activity(object) {
  ...
  if (!this.state._addOnInfos) {
    this.state._addOnInfos = {};
  }
  return this;
}
...
Activity.prototype.addOn = function () {
  return this.state._addOn || {};
};

Activity.prototype.fillAddOn = function (object) {
  var self = this;

  function setOrDelete(fieldname) {
    if (object[fieldname]) {
      self.state._addOn[fieldname] = true;
    } else {
      delete self.state._addOn[fieldname];
    }
  }
  ...
  setOrDelete('home_address');
  setOrDelete('billing_address');
  setOrDelete('t_shirt_size');
  setOrDelete('roommate');
  return self;
};

Activity.prototype.showAddOnLink = function () {
  return !!this.state._addOn;
};

// AddOn information as entered by the participants:
Activity.prototype.addOnInfoOfMember = function (memberId) {
  if (!this.state._addOnInfos[memberId]) {
    this.state._addOnInfos[memberId] = {};
  }
  return new AddOnInfoOfMember(this.state._addOnInfos[memberId]);
};
