...
function Activity(object) {
  if (object) {
    ...
    if (!this.state.resources) {
      this.state.resources = {default: {_registeredMembers: misc.toArray(this.state.registeredMembers)}};
...
function ensureList(thing) {
  // when the request contains only one instance of a parameter,
  // bodyParser does not know that we want this parameter to be a list,
  // and thus returns a plain string
  if (typeof(thing) === 'string') { return [thing]; }
  else { return thing; }
}

function parseResources(body) {
  // bodyParser does not support nested dicts; nested lists is the best
  // it can do, so we give it a hand here.
  return _.object(ensureList(body['resources']['names']),
    ensureList(body['resources']['limits']));
}
...
Activity.prototype.fillFromUI = function (object) {
  ...
  if (object.resources) {
    object['resources'] = parseResources(object);
    _.each(object.resources, function (limit, name) {
      if (!_.has(self.state.resources, name)) {
        self.state.resources[name] = {_registeredMembers: []};
      }
      var intLimit = parseInt(limit, 10);
      if (intLimit && intLimit > 0) {
        self.state.resources[name]['_limit'] = intLimit;
      }
    });
    _.each(self.state.resources, function (resource, name) {
      if (!_.has(object.resources, name)) {
        delete(self.state.resources[name]);
      }
    });
  }
...
Activity.prototype.resourceNames = function (includeDefault) {
  return this.resources().resourceNames(includeDefault);
};
