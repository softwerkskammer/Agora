...
module.exports = {
  ...
  addOnInfoOfMember: function (activityUrl, memberId, callback) {
    activitystore.getActivity(activityUrl, function (err, activity) {
      if (err || !activity) { return callback(err); }
      return callback(null, activity.addOnInfoOfMember(memberId), activity.addOn());
    });
  },

  saveAddOnInfoOfMember: function (activityUrl, memberId, uiInputObject, callback) {
    var self = this;
    activitystore.getActivity(activityUrl, function (err, activity) {
      if (err || !activity) { return callback(err); }
      activity.addOnInfoOfMember(memberId).fillFromUI(uiInputObject);
      return activitystore.saveActivity(activity, function (err) {
        if (err && err.message === "Conflicting versions.") {
          // we try again because of a racing condition during save:
          return self.saveAddOnInfoOfMember(activityUrl, memberId, uiInputObject, callback);
        }
        return callback(err);
      });
    });
