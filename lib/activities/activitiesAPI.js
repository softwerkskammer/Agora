...
function nowIfUndefined(date) {
  return typeof date !== 'undefined' ? date : moment();
}

function toUTC(moment) {
  var zoneOffsetInMinutes = moment.clone().local().zone();
  return moment.clone().subtract('m', zoneOffsetInMinutes).utc();
}
...
module.exports = {
  ...
  upcomingActivities: function (callback, now) {
    var nowInUTC = toUTC(nowIfUndefined(now));
    var start = nowInUTC.unix();
    var end = nowInUTC.add('y', 10).unix();
