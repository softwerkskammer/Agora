...
function Resource(resourceObject) {
  this.state = resourceObject; // this must be *the* object that is contained in activity.resources[resourceName]
  return this;
}
...
Resource.prototype.isFull = function () {
  if (this.state && this.state._limit) {
    return this.state._limit === this.registeredMembers().length;
  }
  return false;
};
