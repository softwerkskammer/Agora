...
module.exports = {
  ...
  activityAndAddonLines: function (activityUrl, callback) {
    activitystore.getActivity(activityUrl, function (err, activity) {
      if (err || !activity) { return callback(err); }
      membersAPI.getMembersForIds(activity.allRegisteredMembers(), function (err, members) {
        if (err) { return callback(err); }
        var addonLines = _.map(members, function (member) {
          var addon = activity.addonForMember(member.id());
          member.addon = addon;
          return member;
        });
        callback(null, activity, addonLines);
      });
    });
  }
