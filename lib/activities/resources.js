...
Resources.prototype.allWaitinglistEntries = function () {
  ...
  var addResourceName = function (waitinglistEntry) {
    return waitinglistEntry._resourceName = key;
  };
  for (var key in this.state) {
    var waitinglistEntries = this.named(key).waitinglistEntries();
    _.each(waitinglistEntries, addResourceName);
    allWaitinglistEntries = allWaitinglistEntries.concat(waitinglistEntries);
  }
  return _.uniq(allWaitinglistEntries);
};
