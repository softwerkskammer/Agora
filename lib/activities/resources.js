...
Resources.prototype.registrationDatesOf = function (member) {
  var self = this;
  return _(self.resourceNames()).map(function (resourceName) { return self.named(resourceName).registrationDateOf(member.id()); }).compact().sortBy().value();
};

Resources.prototype.resourceNamesOf = function (member) {
  var self = this;
  return _(self.resourceNames()).map(function (resourceName) {
    return self.named(resourceName).isAlreadyRegistered(member.id()) ? resourceName : undefined;
  }).compact().sort().value();
};

Resources.prototype.memberIsRegisteredForMoreDaysThan = function (days, member) {
  var soManyDaysAgo = moment().subtract('days', days);
  return soManyDaysAgo.isAfter(this.registrationDatesOf(member).sort()[0]);
};
