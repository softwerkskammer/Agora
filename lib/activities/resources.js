...
function matchArrayEntries(input) {
  return _.zip(misc.toArray(input.names), misc.toArray(input.limits), misc.toArray(input.previousNames),
...
Resources.prototype.fillFromUI = function (uiInputArrays) {
  ...
  _.each(newResources, function (input) {
    ...
    if (resource) {
      ...
      // store additional settings in the new resource
      resource._registrationOpen = registrationOpen;
      resource._withWaitinglist = withWaitinglist;
    }
