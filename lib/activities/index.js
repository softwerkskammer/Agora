...
module.exports = function (app) {
    ...
    activitystore.getActivity(req.body.previousUrl, function (err, activity) {
      ...
      activitystore.saveActivity(activity, function (err) {
        if (err && err.message === "Conflicting versions.") {
          ...
          statusmessage.errorMessage(req, 'message.title.conflict', 'message.content.save_error_retry');
  ...
  app.get('/editAddonConfig/:url', function (req, res, next) {
    activitiesAPI.getActivityWithGroupAndParticipants(req.params.url, function (err, activity) {
      if (err || activity === null) { return next(err); }
      if (!res.locals.accessrights.canEditActivity(activity)) {
        return res.redirect('/activities/' + encodeURIComponent(req.params.url));
      }
      res.render('editAddonConfig', { activity: activity });
    });
  });

  app.post('/submitAddonConfig', function (req, res, next) {
    activitystore.getActivity(req.body.url, function (err, activity) {
      if (err || !activity) { return next(err); }
      activity.fillAddonConfig(req.body);
      activitystore.saveActivity(activity, function (err) {
        if (err && err.message === "Conflicting versions.") {
          // we try again because of a racing condition during save:
          statusmessage.errorMessage(req, 'message.title.conflict', 'message.content.save_error_retry');
          return res.redirect('/activities/editAddonConfig/' + encodeURIComponent(activity.url()));
        }
        if (err) { return next(err); }
        statusmessage.successMessage(req, 'message.title.save_successful', 'message.content.activities.saved_addon_config');
        res.redirect('/activities/' + encodeURIComponent(activity.url()));
      });
    });
  });
  ...
  app.get('/addon/:url', function (req, res, next) {
    ...
    activitiesAPI.addonForMember(url, req.user.member.id(), function (err, addon, addonConfig) {
      if (err) { return next(err); }
  ...
  app.post('/submitAddon', function (req, res, next) {
    ...
    activitiesAPI.saveAddon(url, req.user.member.id(), req.body, function (err) {
      if (err) { return next(err); }
  ...
  app.get('/:url', function (req, res, next) {
    activitiesAPI.getActivityWithGroupAndParticipants(req.params.url, function (err, activity) {
      if (err || !activity) { return next(err); }
      res.render('get', { activity: activity, resourceRegistrationRenderer: resourceRegistrationRenderer, calViewYear: activity.year(), calViewMonth: activity.month(), canEditAddon: activity.hasAddonConfig()});
    });
  });
