    ...
    activitiesAPI.getActivityWithGroupAndParticipants(req.params.url, function (err, activity) {
      ...
      addonAPI.addonLinesOf(activity, function (err, addonLines) {
        ...
        var formatDates = function (dates) {
          return _(dates).map(function (date) { return date.lang(res.locals.language).format('L'); }).uniq().value().join(', ');
        };
        res.render('managementTables', {activity: activity, addonLines: addonLines, formatDates: formatDates});
      });
    });
  });
