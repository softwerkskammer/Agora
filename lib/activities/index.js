...
app.get('/:url', function (req, res, next) {
  activitiesService.getActivityWithGroupAndParticipants(req.params.url, function (err, activity) {
    ...
    memberstore.getMembersForIds(activity.editors(), function (err, editors) {
      if (err || !editors) {
        return next(err);
      }
      var editorNicknames = _.map(editors, function (editor) { return editor.nickname(); });
      res.render('get', { activity: activity, editors: editorNicknames, resourceRegistrationRenderer: resourceRegistrationRenderer,
