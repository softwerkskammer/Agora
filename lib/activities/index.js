...
module.exports = function (app) {
  ...
  app.post('/submit', function (req, res, next) {
    ...
    async.parallel([
      function (callback) {
        validation.checkValidity(req.body.previousUrl, req.body.url, activitiesCoreAPI.isValidUrl, 'Diese URL ist leider nicht verfügbar.', callback);
      },
      function (callback) {
        var activity = new Activity(req.body);
  ...
  app.get('/subscribe/:url', function (req, res, next) {
    activitiesCoreAPI.addVisitorTo(req.user.member.id, req.params.url, function (err) {
      if (err) { return next(err); }
  ...
  app.get('/unsubscribe/:url', function (req, res, next) {
    activitiesCoreAPI.removeVisitorFrom(req.user.member.id, req.params.url, function (err) {
      if (err) { return next(err); }
      res.redirect('/activities/' + encodeURIComponent(req.params.url));
    });
  });
