...
module.exports = function (app) {
  ...
  app.get('/newLike/:url', function (req, res, next) {
    activitystore.getActivity(req.params.url, function (err, activity) {
      ...
      renderActivityCombinedWithGroups(res, next, new Activity().copyFrom(activity));
    });
  });
  ...
  app.get('/addToWaitinglist/:activityUrl/:resourceName', function (req, res, next) {
    var args = {nickname: req.user.member.nickname, activityUrl: req.params.activityUrl, resourcename: req.params.resourceName};
    // TODO here we might as well define a function that gets the member id from req.user.member.id
    waitinglistAPI.saveWaitinglistEntry(args, function (err) {
      if (err) { return next(err); }
      statusmessage.successMessage('Eintragen in die Warteliste erfolgreich', '(Bitte ignoriere, dass der Button immer noch "Auf die Warteliste" anzeigt... wir arbeiten dran!)').putInSession(req);
