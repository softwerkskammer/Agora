  ...
  app.post('/submitAddon', function (req, res, next) {
    ...
    activitiesAPI.getActivityWithGroupAndParticipants(url, function (err, activity) {
      ...
      activitiesAPI.saveAddon(url, req.user.member.id(), req.body, function (err) {
        if (err) { return next(err); }
        statusmessage.successMessage(req, 'message.title.save_successful', 'message.content.addon.saved');
  ...
  app.get('/:url', function (req, res, next) {
    activitiesAPI.getActivityWithGroupAndParticipants(req.params.url, function (err, activity) {
      ...
      res.render('get', { activity: activity, resourceRegistrationRenderer: resourceRegistrationRenderer,
        calViewYear: activity.year(), calViewMonth: activity.month(), canEditAddon: activity.hasAddonConfig() &&
          req.user && _.find(activity.participants, function (participant) {
            return participant.id() === req.user.member.id();
          })});
    });
  });
