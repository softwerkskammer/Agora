...
module.exports = function (app) {
  ...
  function activitySubmitted(req, res, next) {
    activitiesCoreAPI.getActivity(req.body.previousUrl, function (err, activity) {
      ...
      req.body['resources'] = parseResources(req.body);
  ...
  app.get('/:url', function (req, res, next) {
    activitiesAPI.getActivityWithGroupAndParticipants(req.params.url, function (err, activity) {
      ...
      res.render('get', { activity: activity, calViewYear: activity.year(), calViewMonth: activity.month()});
    });
  });
  ...
  app.get('/subscribe/:url/:resource', function (req, res, next) {
    activitiesCoreAPI.addVisitorTo(req.user.member.id, req.params.url, req.params.resource, function (err) {
      if (err) { return next(err); }
      if (req.params.resource === 'default') {
        statusmessage.successMessage('Speichern erfolgreich', 'Deine Teilnahme wurde eingetragen.').putInSession(req);
      } else {
        statusmessage.successMessage('Speichern erfolgreich', 'Deine Teilnahme wurde für "' + req.params.resource + '" eingetragen.').putInSession(req);
      }
  ...
  app.get('/subscribe/:url', function (req, res) {
    ...
    res.redirect('/activities/subscribe/' + encodeURIComponent(req.params.url) + '/default');
  });
