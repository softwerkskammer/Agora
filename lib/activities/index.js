...
module.exports = function (app) {
  ...
  app.post('/payment/submitCreditCard', function (req, res, next) {
    var url = req.body.url;
    addonAPI.payWithCreditCard(url, req.user.member.id(), req.body.stripeId, function (err, charge) {
      if (err) { return next(err); }
  ...
  app.get('/paymentReceived/:activityUrl/:nickname', function (req, res, next) {
    addonAPI.submitPaymentReceived(req.params.activityUrl, req.params.nickname, function (err) {
      if (err) { return next(err); }
      res.redirect('/activities/addons/' + encodeURIComponent(req.params.activityUrl));
    });
  });
