...
module.exports = function (app) {
  ...
  app.get('/eventsForSidebar', function (req, res, next) {
    ...
    api.allActivities(function (err, activities) {
      ...
      var fittingActivities = activities.filter(function (activity) {
        return activity.startUnix > moment.unix(req.query.start).unix() && activity.startUnix < moment.unix(req.query.end).unix();
      });
      res.end(JSON.stringify(fittingActivities.map(function (activity) {
        return {
          title: activity.title,
          start: activity.startUnix,
          url: '/activities/' + activity.id,
          activity: activity,
          dayOfWeek: moment.unix(activity.startUnix).day(),
          className: 'verySmall'
        };
      })));
    });
