...
module.exports = function (app) {
  ...
  app.post('/payment/submitTransfer', function (req, res, next) {
    ...
    activitiesAPI.payWithTransfer(url, req.user.member.id(), function (err) {
      if (err) { return next(err); }
  ...
  app.post('/payment/submitCreditCard', function (req, res, next) {
    var url = req.body.url;
    activitiesAPI.payWithCreditCard(url, req.user.member.id(), req.body.stripeId, function (err, charge) {
      if (err) { return next(err); }
      var displayAmount = charge.amount / 100 + ' €';
      statusmessage.successMessage('message.title.save_successful', 'message.content.activities.credit_card_paid', {amount: displayAmount}).putIntoSession(req);
      res.redirect('/activities/' + encodeURIComponent(url));
    });
  });
