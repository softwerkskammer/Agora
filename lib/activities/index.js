...
function activitiesForDisplay(activitiesFetcher, next, res, title) {
  return activitiesAPI.getActivitiesForDisplay(activitiesFetcher, function (err, activities) {
    if (err) { next(err); }
...
app.get('/checkurl', function (req, res) {
  misc.validate(req.query.url.trim(), req.query.previousUrl.trim(), _.partial(activitiesAPI.isValidUrl, reservedURLs), res.end);
});
...
app.get('/addons/:url', function (req, res, next) {
  activitiesAPI.getActivityWithGroupAndParticipants(req.params.url, function (err, activity) {
    ...
    groupsAndMembersAPI.addMembersToGroup(activity.group, function (err) {
      if (err) { return next(err); }
      ...
      addonAPI.addonLinesOf(activity, function (err, addonLines) {
        ...
        res.render('managementTables', {activity: activity, addonLines: addonLines, tshirtsizes: addonAPI.tshirtSizes(addonLines), containsMember: containsMember, formatDates: formatDates, formatList: formatList});
      });
    });
  });
});
