...
module.exports = function (app) {
  ...
  var api = beans.get('activitiesAPI');
  var activitiesGCAPI = beans.get('activitiesGroupsColorsAPI');
  ...
  function activitySubmitted(req, res, next) {
    ...
    api.saveActivity(activity, function (err, savedActivity) {
  ...
  function activitiesForDisplay(activitesFetcher, next, res, title) {
    return activitiesGCAPI.getActivitiesForDisplay(activitesFetcher, function (err, activities) {
      if (err) { next(err); }
  ...
  app.get('/', function (req, res, next) {
    activitiesForDisplay(api.allActivities, next, res, 'Alle');
  });

  app.get('/upcoming', function (req, res, next) {
    activitiesForDisplay(api.upcomingActivities, next, res, 'Kommende');
  });

  app.get('/past', function (req, res, next) {
    activitiesForDisplay(api.pastActivities, next, res, 'Vergangene');
  });
  ...
  app.post('/submit', function (req, res, next) {
    ...
    async.parallel([
      function (callback) {
        validation.checkValidity(req.body.previousUrl, req.body.url, api.isValidUrl, 'Diese URL ist leider nicht verfügbar.', callback);
      },
      function (callback) {
        var activity = new Activity(req.body);
