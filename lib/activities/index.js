...
module.exports = function (app) {
  ...
  app.post('/submit', function (req, res, next) {
    ...
    async.parallel([
      function (callback) {
        ...
        var validityChecker = function (url, callback) { activitiesCoreAPI.isValidUrl(isReserved, url, callback); };
        validation.checkValidity(req.body.previousUrl, req.body.url, validityChecker, req.i18n.t("validation.url_not_available"), callback);
      },
      function (callback) {
  ...
  app.get('/checkurl', function (req, res) {
    ...
    activitiesCoreAPI.isValidUrl(isReserved, url, function (err, result) {
  ...
  app.get('/addToWaitinglist/:activityUrl/:resourceName', function (req, res, next) {
    activitiesCoreAPI.addToWaitinglist(req.user.member.id, req.params.activityUrl, req.params.resourceName, moment(), function (err, statusTitle, statusText) {
      if (err) { return next(err); }
      if (statusTitle && statusText) {
        statusmessage.errorMessage(statusTitle, statusText).putInSession(req);
      } else {
