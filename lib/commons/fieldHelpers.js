...
module.exports = {
  ...
  replacePhoneNumbers: function (text) {
    if (text) {
      return text.replace(/\+?(\d+\s?)?(\(?\s?(\d+\s?)+(\)|\/|-))?(\s?\d{2,}){2,}/g, '...');
      // this means: maybe countryCode, maybe preDial, at least two blocks of two or more digits, maybe separated by spaces
      // preDial: maybe (, maybe space, one or more digits, potentially mixed with spaces, followed by ) or - or /
    }
    return text;
  },
