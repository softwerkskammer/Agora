...
module.exports = {
  ...
  readableDate: function (unixtimestamp) {
    if (unixtimestamp) {
      if (moment(unixtimestamp.toString(), 'DD.MM.YYYY').isValid()) {
        // Keep German dates from activities as they are
        return unixtimestamp;
      }
      if (unixtimestamp && moment.unix(unixtimestamp).isValid()) {
        return moment.utc(unixtimestamp.toString(), 'X').format('DD.MM.YYYY');
      } else {
        return null;
      }
    } else {
