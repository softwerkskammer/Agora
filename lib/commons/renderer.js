...
function evalTags(text, subdir) {
  ...
  for (var k in tagmap) {
    ...
    tagmap[k] = '<a class="internal" href="/wiki/' + (subdir ? subdir : 'alle') + '/' + normalise(pageName.toLowerCase()) + '">' + name + '</a>';
  }
...
var Renderer = {
  render: function (content, subdir) {
    ...
    return rendered.replace('<table>', '<table class="table table-condensed table-hover table-striped">').replace('<img src=', '<img class="img-responsive" src=');
  },
