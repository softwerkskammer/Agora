...
module.exports = {
  ...
  isValidGroup: function (group) {
    ...
    validator.check(group.id, 'Name darf nur Buchstaben, Zahlen, Bindestrich und Unterstrich enthalten.').regex(/^[\w-ßüäö]+$/i);
  ...
  isValidActivity: function (activity) {
    ...
    validator.check(activity.endDate(), 'Endedatum ist ein Pflichtfeld.').notEmpty();
    validator.check(activity.endTime(), 'Enduhrzeit ist ein Pflichtfeld.').notEmpty();
    if (activity.startUnix >= activity.endUnix) {
