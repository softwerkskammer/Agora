...
var misc = require('nconf').get('beans').get('misc');
...
module.exports = {
  ...
  isValidForActivity: function (activityInput) {
    ...
    if (nonEmptyResourceNames.length === 0) {
      additionalErrors.push('Es muss mindestens eine Ressourcenbezeichnung angegeben werden.');
    }
    if (nonEmptyResourceNames.length !== _.uniq(nonEmptyResourceNames).length) {
      additionalErrors.push('Die Bezeichnungen der Ressourcen müssen eindeutig sein.');
    }


//    if (activityInput.startUnix() >= activityInput.endUnix()) {
//      validator.error('Start muss vor Ende liegen.');
//    }
    return _.union(validator.getErrors(), additionalErrors);
  },
