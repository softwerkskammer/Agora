...
module.exports = function redirectIfNotSuperuser(req, res, next) {
  ...
  if (isAuthenticated) {
    if (/members\/edit/.test(originalUrl)) {
      userNeedsNotBeSuperuser = '/members/edit/' + encodeURIComponent(user.member.nickname()) === originalUrl;
      logger.info('Test, ob die Bearbeitung des eigenen Profils stattfindet:');
      logger.info('Nickname: ' + user.member.nickname());
      logger.info('Gesuchte URL: ' + '/members/edit/' + encodeURIComponent(user.member.nickname()));
      logger.info('Original URL: ' + originalUrl);
      logger.info('Ergebnis: ' + userNeedsNotBeSuperuser);
    }
