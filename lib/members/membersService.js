...
function ungroupedWords(members) {
...
module.exports = {
  ...
  toUngroupedWordList: function (members) {
    return ungroupedWords(members)
      .groupBy() // prepare counting by grouping
      .transform(function (result, words) {
        return result.push({text: words[0], weight: words.length, link: '/members/interests?interest=' + words[0]});
      }, []) // create the final structure
      .sortBy('text')
      .value();
  },
