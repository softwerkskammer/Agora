...
Member.prototype.updateWith = function (object, sessionUser) {
  if (sessionUser) {
    ...
    this.firstname = sessionUser.profile.name.givenName;
    this.lastname = sessionUser.profile.name.familyName;
    this.email = sessionUser.profile.emails[0].value;
  }
  if (object) {
    this.firstname = isFilled(object.firstName) ? object.firstname : this.firstname;
    this.lastname = isFilled(object.firstName) ? object.lastname : this.lastname;
    this.nickname = object.nickname;
    this.email = isFilled(object.email) ? object.email : this.email;
    this.twitter = object.twitter;
    this.location = object.location;
    this.profession = object.profession;
    this.interests = object.interests;
