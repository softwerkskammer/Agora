...
var logger = winston.loggers.get('memberstore');
...
module.exports = {
  ...
  getMembersForEMails: function (emails, callback) {
    ...
    async.map(emails, function (item, cb) { store.getMemberForEMail(item, cb); }, function (err, results) {
      ...
      var missingEmails = _.reject(_.pluck(results, 'email'), function (email) {
        return _.contains(emails, email);
      });
      if (_.size(missingEmails) > 0) {
        logger.warn("Missing emails " + missingEmails);
      }
