...
var sortCaseInsensitive = function (objectlist) {
  return objectlist.sort(function (a, b) {
    var low_last_a = a.lastname.toLowerCase();
    var low_last_b = b.lastname.toLowerCase();

    if (low_last_a === low_last_b) {
      var low_first_a = a.firstname.toLowerCase();
      var low_first_b = b.firstname.toLowerCase();

      if (low_first_a === low_first_b) {
        return 0;
      }
      return low_first_a < low_first_b ? -1 : 1;
    }
    return low_last_a < low_last_b ? -1 : 1;
  });
