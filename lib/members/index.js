...
module.exports = function (app) {
  ...
  app.get('/dashboard/:nickname', function (req, res, next) {
    var activitiesFetcher = function (subscribedGroups) {
      var groupIds = _.pluck(subscribedGroups, "id");
      return _.curry(activitystore.upcomingActivitiesForGroupIds)(groupIds);
    };

    groupsAndMembersAPI.getUserWithHisGroups(req.params.nickname, function (err, member) {
      if (err || !member) { return next(err); }
      activitiesAPI.getActivitiesForDisplay(activitiesFetcher(member.subscribedGroups), function (err, activities) {
        if (err || !activities) { next(err); }
