...
module.exports = function (app) {
  ...
  app.get('/:nickname', function (req, res, next) {
    groupsAndMembersAPI.getUserWithHisGroups(req.params.nickname, function (err, member, subscribedGroups) {
      if (err || !member) { return next(err); }
      res.render('get', { member: member, subscribedGroups: subscribedGroups });
    });
  });

  app.get('/dashboard/:nickname', function (req, res, next) {
    var activitiesFetcher = function (subscribedGroups, memberId) {
      var groupIds = _.pluck(subscribedGroups, "id");
      //return _.partial(activitystore.upcomingActivitiesForGroupIds, groupIds);
      return _.partial(activitystore.upcomingActivitiesForGroupIdsAndRegisteredMemberId, groupIds, memberId);
    };

    groupsAndMembersAPI.getUserWithHisGroups(req.params.nickname, function (err, member) {
      if (err || !member) { return next(err); }
      activitiesAPI.getActivitiesForDisplay(activitiesFetcher(member.subscribedGroups, member.id()), function (err, activities) {
        if (err || !activities) { return next(err); }

        res.render('dashboard', { member: member, activities: activities });
      });
    });
  });
