    ...
    async.parallel(
      ...
      function (err, results) {
        ...
        res.render('edit', { member: memberForNew(req),
          markedGroups: groupsAPI.combineSubscribedAndAvailableGroups([results.alle, results.commercial], results.allGroups) });
      });
  });
  ...
  app.get('/edit/:nickname', function (req, res, next) {
    ...
    groupsAndMembersAPI.getUserWithHisGroups(nicknameOfEditMember, function (err, member, subscribedGroups) {
      ...
      groupsAPI.getAllAvailableGroups(function (err, allGroups) {
        if (err) { return next(err); }
        res.render('edit', { member: member, markedGroups: groupsAPI.combineSubscribedAndAvailableGroups(subscribedGroups, allGroups) });
      });
    });
  });
