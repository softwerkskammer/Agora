...
module.exports = function (app) {
  ...
  function saveMember(memberOfRequest, previousMemberData, req, res, next) {
    api.saveMember(memberOfRequest, function (err) {
      if (err) { return next(err); }
      if (!req.user.member || req.user.member.id === memberOfRequest.id) {
        req.user.member = memberOfRequest;
        delete req.user.profile;
      }

      var oldEmail = previousMemberData ? previousMemberData.email : memberOfRequest.email;
      var subscriptions = misc.toArray(req.body.newSubscriptions);
      if (memberOfRequest.isAdmin) {
        var adminListName = conf.get('adminListName');
        if (adminListName) {
          subscriptions.push(adminListName);
        }
      }
      return groupsAPI.updateSubscriptions(memberOfRequest.email, oldEmail, subscriptions, function (err) {
        if (err) { return next(err); }
        statusmessage.successMessage('message.title.save_successful', 'message.content.members.saved').putInSession(req);
        return res.redirect('/members/' + encodeURIComponent(memberOfRequest.nickname));
      });
    });
  }
