  ...
  app.post('/submit', function (req, res, next) {
    ...
    async.parallel([
      function (callback) {
        ...
        if (nickname !== previousNickname) {
          return api.isValidNickname(nickname, function (err, check) {
            if (err || !check) {
              return callback(null, 'Dieser Nickname ist leider nicht verfügbar.');
            }
          });
        }
      ...
      function (callback) {
        ...
        var previousEmail = req.body.previousEmail;
        if (email !== previousEmail) {
          return api.isValidEmail(email, function (err, check) {
            if (err || !check) {
              return callback(null, 'Diese Adresse ist schon registriert. Hast Du bereits ein Profil angelegt?');
            }
          });
        }
        return callback(null);
      ...
      function (err, errorMessages) {
        var realErrors = _.filter(errorMessages, function (message) { return !!message; });
