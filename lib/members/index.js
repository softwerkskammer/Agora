...
module.exports = function (app) {
  ...
  app.get('/dashboard/:nickname', function (req, res, next) {
    var activitiesFetcher = function (member) {
      ...
      return _.partial(activitystore.upcomingActivitiesForGroupIdsAndRegisteredMemberId, groupIds, member.id());
    };

    groupsAndMembersAPI.getUserWithHisGroups(req.params.nickname, function (err, member) {
      if (err || !member) { return next(err); }
      activitiesAPI.getActivitiesForDisplay(activitiesFetcher(member), function (err, activities) {
