...
module.exports = function (app) {
  ...
  app.get('/:nickname', function (req, res, next) {
    groupsAndMembersAPI.getUserWithHisGroups(req.params.nickname, function (err, member, subscribedGroups) {
      if (err || !member) { return next(err); }
      activitiesAPI.getPastActivitiesOfMember(member, function (err, activities) {
        if (err) { return next(err); }
    ...
    groupsAndMembersAPI.getUserWithHisGroups(req.params.nickname, function (err, member) {
      ...
      activitiesAPI.getUpcomingActivitiesOfMemberAndHisGroups(member, function (err, activities) {
        ...
        async.each(member.subscribedGroups, function (group, callback) {
          ...
          if (err) { return next(err); }
          res.render('dashboard', { member: member, activities: activities, postsByGroup: postsByGroup, changesByGroup: changesByGroup });
