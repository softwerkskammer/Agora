  ...
  function saveMember(persistentMember, req, res, next) {
    ...
    api.saveMember(member, function (err) {
      ...
      return groupsAPI.updateSubscriptions(member.email(), oldEmail, subscriptions, function (err) {
        if (err) { return next(err); }
        groupsAndMembersAPI.getMemberWithHisGroupsByMemberId(member.id(), function (err, member) {
          groupsAndMembersAPI.updateAdminlistSubscriptions(member, function (err) {
            if (err) { return next(err); }
